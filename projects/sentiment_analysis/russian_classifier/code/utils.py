...
try:
    _base_stopwords = set(stopwords.words("russian"))  # O(1) membership checks.
...
_NON_TEXT_RE = re.compile(r"[^a-zа-яё0-9\s']")
...
def clean_text(text):  # Normalize and clean Russian text.
    ...
    text = _NON_TEXT_RE.sub(" ", text)  # Keep Russian/Latin letters, digits, spaces, apostrophes.
    text = _WS_RE.sub(" ", text).strip()  # Collapse repeated whitespace.
